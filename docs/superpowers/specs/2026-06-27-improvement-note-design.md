...
| **개선** (신규) | 상단 `＋ 대화 캡처 올리기` 버튼 + `[전체 | 단어 | 문법]` 세그먼트. **캡처에서 등록한 항목만**(`source==='capture'`). `전체`=단어+문법 전부, `단어`=word/idiom/slang, `문법`=**카테고리별 그룹**(아래 카테고리 절). |
| **기록** | 기존 그대로(연속 학습). |
| **더보기** | 기존 설정 그대로. |
| ~~복습~~ | **제거.** |
...
### 개선 탭 — 전체/단어/문법 + 문법 카테고리

개선 탭 세그먼트는 `전체 | 단어 | 문법` 3개.

- **전체**: `source==='capture'` 항목 전부(단어·표현·문법 합친 한 목록).
- **단어**: `source==='capture' && type!=='grammar'`.
- **문법**: `source==='capture' && type==='grammar'` 를 **카테고리(`grammarCat`)별로 그룹**해 **카테고리 카드**로 보인다(저장된 카테고리만, 각 개수 표시). 카드 탭 → 그 카테고리 항목 목록(`‹ 분류 전체`로 복귀). 2단계 뷰.

**카테고리 부여(AI 자동):** 캡처 분석 시 Gemini가 각 문법 약점에 한국어 카테고리명을 함께 반환한다(예: `전치사`, `수일치`, `시제`, `관사`, `복수형`, `어순`, `관계사`, `동사형태`, `접속사` 등 자유 텍스트). 저장 시 `grammarCat`로 보관. 값이 없으면 `기타`로 묶는다. 고정 사전 없이 AI가 만든 카테고리명을 그대로 그룹 키로 쓴다 — 저장된 카테고리만 목록에 나타난다.
...
```js
{
  ...
  grammarCat: '전치사',            // grammar 항목의 AI 자동 분류(없으면 기타) ← 추가
...
이미지는 영어 튜터 앱의 "개선할 점/수정 제안" 화면이야. JSON으로만 답해:
{
 ...
 "weaknesses":[
   {"type":"grammar|word|idiom","term":"항목 이름(문법은 규칙명, 표현은 표제어형)",
    ...
    "category":"문법 항목이면 한국어 분류명(전치사/수일치/시제/관사/복수형/어순/관계사/동사형태 등). 단어·표현이면 빈 문자열"}
 ]
}
...
저장 시 grammar 항목은 `grammarCat = weakness.category || '기타'`로 보관한다.
