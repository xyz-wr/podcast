...
// index.html은 단일 파일 앱이라 로직을 별도 모듈로 분리하지 않는다.
...
export function loadFolders(){
  const html = fs.readFileSync(HTML_PATH, 'utf8');
  const m = html.match(/<script id="conv-data" type="application\/json">([\s\S]*?)<\/script>/);
  if(!m) throw new Error('index.html에서 data 스크립트를 찾지 못했습니다');
